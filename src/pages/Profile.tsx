...
import {
  ...
  Award, Lightbulb, MapPin, ExternalLink, LogOut, UserPlus, Folder, Trash2
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* === BLOGS & READING LISTS SECTION === */}
        <div>
          {/* Tab Selector */}
          <div className="flex space-x-6 border-b border-gray-200 dark:border-gray-800 mb-8">
            <button
              onClick={() => setActiveTab('blogs')}
              className={`pb-4 text-xl font-bold border-b-2 transition-all cursor-pointer flex items-center space-x-2
                ${activeTab === 'blogs' 
                  ? 'border-blue-600 text-blue-600 dark:text-blue-400' 
                  : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
            >
              <FileText className="w-5 h-5" />
              <span>My Blogs ({userBlogs.length})</span>
            </button>
            <button
            ...
            loadingLists ? (
              ...
              <div className="space-y-6">
                {readingLists.map(list => (
                  <div key={list.id} className="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-md">
                    ...
                    {list.blogs && list.blogs.length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {list.blogs.map(blog => (
                          <div key={blog.id} className="p-4 bg-gray-50 dark:bg-gray-800/40 rounded-xl border border-gray-150 dark:border-gray-750 flex items-center justify-between gap-4">
                            <div className="min-w-0">
                              <Link to={`/blog/${blog.slug}`} className="font-semibold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 truncate block">
                                {blog.title}
                              </Link>
                              <span className="text-xs text-gray-500 dark:text-gray-400">By {blog.author?.full_name}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-sm text-gray-500 dark:text-gray-400 italic">This reading list is empty.</p>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-12 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Save blogs to lists to read them later or curate your favorite posts.
                </p>
              </div>
            )
          )}
